    ...
    <section id="home" className="section-padding relative flex min-h-screen items-center overflow-hidden pt-28">
      ...
      <div className="mx-auto grid w-full max-w-7xl items-center gap-12 px-4 sm:px-6 lg:grid-cols-[1.05fr_0.95fr] lg:px-8">
        <motion.div className="relative z-10" initial={{ opacity: 0, y: 36 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.75 }}>
          ...
          <div className="mb-6 inline-flex max-w-full items-center gap-3 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm font-semibold leading-6 text-cyan-200 backdrop-blur-xl">
            <span className="size-2 shrink-0 rounded-full bg-emerald-300 shadow-[0_0_18px_rgba(110,231,183,.95)]" />
            Available for MERN Stack opportunities
          </div>

          <h1 className="max-w-4xl text-[clamp(2.75rem,14vw,4rem)] font-black leading-tight text-white sm:text-6xl lg:text-7xl">
            Mithun <span className="gradient-text">Talukdar</span>
          </h1>
          <div className="hero-role-line mt-5 min-h-16 text-[clamp(1.15rem,5.8vw,1.5rem)] font-bold text-slate-200 sm:text-3xl">
            <span className="typing-gradient">Full Stack Web Developer</span>
            <span className="mx-2 text-cyan-300">|</span>
            <span>MERN Stack Developer</span>
          </div>
          <p className="mt-6 max-w-2xl text-[clamp(1rem,3.6vw,1.125rem)] leading-8 text-slate-300 sm:leading-9">{profile.tagline}</p>
